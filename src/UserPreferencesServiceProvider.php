...
class UserPreferencesServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        if ($this->app->runningInConsole()) {
            ...
            if (! class_exists('AddPreferencesToUsersTable')) {
                $this->publishes([
                    __DIR__ . '/database/migrations/add_preferences_to_users_table.php.stub'
                    => database_path('migrations/' . date('Y_m_d_His', time()) . '_add_preferences_to_users_table.php'),
                ], 'migrations');
            }
        }
    }
